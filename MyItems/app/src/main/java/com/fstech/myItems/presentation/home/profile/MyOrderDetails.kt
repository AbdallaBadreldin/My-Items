...
class MyOrderDetails(
        ...
        viewModel.orderDetails.observe(viewLifecycleOwner) {
            if (it != null) {
                ...
                binding.itemsId.text =
                    getString(R.string.items_count, it.data.order_products_new.size.toString())
                binding.priceItems.text =
                    getString(R.string.egps, (it.data.final_total_amount - it.data.cost).toString())

                binding.deliveryFees.text =
                    getString(R.string.egps, "${it.data.cost}")

                binding.totalPrices.text =
                    getString(R.string.egps, it.data.final_total_amount.toString())
    ...
    private fun checkWaiting() {
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markFirst)
    }

    private fun checkPreparing() {
        binding.statusSecond.setTextColor(resources.getColor(R.color.black_blue))
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markSecond)
    }
    ...
    private fun checkReady() {
        ...
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markSecond)
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markThird)
    }
    ...
    private fun checkOnTheWay() {
        ...
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markThird)
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markFourth)
    }
    ...
    private fun checkDelivered() {
        ...
        binding.statusThird.setTextColor(resources.getColor(R.color.black_blue))
        binding.statusFourth.setTextColor(resources.getColor(R.color.black_blue))
        binding.statusFifth.setTextColor(resources.getColor(R.color.green))
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markSecond)
        Glide.with(this)
            .load(R.drawable.mark_first_a).into(binding.markThird)
    ...
    fun setupListeners() {
        ...
        binding.layoutDeliveryAddress.imageView11.visibility = View.INVISIBLE
