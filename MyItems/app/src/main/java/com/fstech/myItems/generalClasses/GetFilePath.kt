...
object GetFilePath {
    ...
    private fun getRealPathFromURI_API19(context: Context, uri: Uri): String? {
        ...
        if (DocumentsContract.isDocumentUri(context, uri)) {
            ...
            if (isExternalStorageDocument(uri)) {
                ...
                var path:String? =  ""
                var fileName = getDownloadsFilePath(context,uri)
                for (contentUriPrefix in contentUriPrefixesToTry) {
                    try {
                        var file = File(contentUriPrefix + fileName)
                        if(file.exists())
                            path =  contentUriPrefix + fileName
                    } catch (e: Exception) {
                        Log.d(TAG, "getRealPathFromURI_API19: $e")
                    }
                }
                return path
    ...
    fun getDownloadsFilePath(context: Context, uri: Uri?): String? {
